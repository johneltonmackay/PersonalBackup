...
define(['N/record', 'N/search', 'N/runtime', 'N/error', 'N/email', 'N/render', 'N/workflow', 'N/format',  './WNZ_Library.js'], function(record, search, runtime, error, email, render, workflow, format){
    ...
    EndPoint.getInputData = function (){
		...
		log.debug('getInputData arrSearchResults', arrSearchResults)

		if (arrSearchResults.length > 0){
			let checkerResults = checkData(arrSearchResults)
			let checkerReturn = checkItem(arrSearchResults)

			if (checkerResults){
				searchResults.columns.push(
					search.createColumn({
						name: 'formulatext',
						formula: '\'true\'',
						label: 'blnChecker'
		...
		log.debug('getInputData searchResults', searchResults);
	 ...
	 EndPoint.map = function ( context ){
		...
		objRec.objType = '';
		objRec.blnChecker = contextValue.values["formulatext"];
		objRec.returnitem = contextValue.values["formulahtml"];
    ...
    EndPoint.reduce = function (context)
	{
		...
		context.values.forEach(function(value) {
				...
				stChecker = objItem.blnChecker;
				arrReturnItem = objItem.returnitem.split(',');
	    		
	    });
		log.debug(stLogTitle, '>> stChecker:: ' +stChecker);
		...
		context.values.forEach(function(value) {
			...
			if(objItem.objType != 'RtnAuth' && objItem.objType != '') {
			  ...
			  if (stChecker == "true"){
				...
				log.debug('reduce arrReturnItem', arrReturnItem);
				if (!arrReturnItem.includes(objItem.item)) {
					objSummaryItem[objItem.purcon].arrIF.push(objItem.id);
				}
			  }
            }
		...
		for(var xyz in objSummaryItem)
		{
			...
			if(objSummaryItem[xyz].flQtyToAllocate == 0)
			{
				...
				for(var lll in arrIfToProcess)
				{
					...
					log.debug(stLogTitle, '>> IF | Updated Order from RA:: ' +stId);
				}
			...
			else 
			{
				...
				for(var lll in arrIfToProcess)
				{
					...
					log.debug(stLogTitle, '>> ELSE | Updated Order from RA:: ' +stId);
				}
    ...
    */
	...
	function checkData(arrSearchResults) {
		var stLogTitle = "checkData";
		var arrValidator = [];
		var blnValidator = false;
		
		arrSearchResults.forEach(function(value) {
			var objItem = value; // No need to parse, value is already an object
			objItem["Qty Shipped"] = parseInt(objItem["Qty Shipped"]); // Use bracket notation for properties with spaces
			objItem["Qty Shipped"] = Math.abs(objItem["Qty Shipped"]);
			log.debug(stLogTitle, '>> objItem for Validator:: ' + JSON.stringify(objItem));
			
			// Check if objType already exists in arrValidator
			var existingItem = arrValidator.find(item => item.objType == objItem.Type && item.objItem == objItem.Item);
	
			if (existingItem) {
				// If objType exists, update the quantity
				existingItem.objQty += objItem["Qty Shipped"]; // Update property name
			} else {
				// If objType does not exist, add a new object to arrValidator
				arrValidator.push({
					objType: objItem.Type,
					objItem: objItem.Item,
					objQty: objItem["Qty Shipped"], // Update property name
				});
			}
		});
		...
		log.debug('arrValidator highestobjType', highestobjType);
		log.debug('arrValidator highestQty', highestQty);
	
		if (arrValidator.length > 0) {
			if (highestobjType != 'RtnAuth' && highestobjType != '') { // Use strict comparison
				blnValidator = true;
			}
		}
	...
	function checkItem(arrSearchResults) {
		var stLogTitle = "checkItem";
		var arrReturnItem = [];
	
		arrSearchResults.forEach(function(value) {
			var objItem = value; // No need to parse, value is already an object
			log.debug(stLogTitle, '>> objItem for checkItem:: ' + JSON.stringify(objItem));
	
			if (objItem.Type == 'RtnAuth'){
				// Check if objItem already exists in arrValidator
				if (!arrReturnItem.includes(objItem.Item)) {
					arrReturnItem.push(objItem.Item); 
				}
			}
		});
	
		log.debug('checkItem arrReturnItem', JSON.stringify(arrReturnItem));
	
		return arrReturnItem;
	}
